...
import { blankStory, deleteStory, getAccess, getAttempts, getStories, isDemo, saveAttempt, saveStory, signIn, signOut, signUp } from "@/lib/data";
import type { Attempt, Question, Story, StoryActivities } from "@/lib/types";
...
export function StoryApp({ view, month = 1, storyId }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const [allStories, allAttempts] = await Promise.all([
  ...
  async function handleSave(story: Story) {
  ...
  return <div className="app-shell">
    ...
    {loading ? <main className="page loading"><div className="spinner"/><p>Đang mở thư viện truyện...</p></main> : <>
      {view === "home" && <Home stories={published} attempts={attempts} />}
      {view === "month" && <Month month={month} stories={published} completedIds={completedIds} />}
      ...
      {view === "admin" && (access.admin ? <Admin stories={stories} editor={editor} setEditor={setEditor} onSave={handleSave} onDelete={handleDelete} onReload={reload} email={access.email} /> : <Login onLogin={reload} />)}
...
function Home({ stories, attempts }: { stories: Story[]; attempts: Attempt[] }) {
  ...
  return <main className="page">
    ...
    <div className="month-grid">{MONTHS.map((label, index) => { const count = stories.filter(s => s.month === index + 1).length; return <Link href={`/months/${index + 1}`} className="month-card" key={label} style={{ "--card-color": MONTH_COLORS[index] } as React.CSSProperties}><div className="month-art"><span className="month-number">{String(index + 1).padStart(2, "0")}</span><span className="month-symbol">{["✿", "☁", "✾", "❀", "☀", "✦", "☼", "✳", "❁", "◕", "✷", "❄"][index]}</span></div><div className="month-meta"><div><h3>{label}</h3><p>{DAYS_IN_MONTH[index]} ngày · {count} truyện đã có</p></div><span className="circle-arrow">↗</span></div></Link>; })}</div>
...
function Month({ month, stories, completedIds }: { month: number; stories: Story[]; completedIds: Set<string> }) {
  ...
  return <main className="page subpage"><div className="breadcrumb"><Link href="/">Trang chủ</Link><span>›</span><span>Thư viện truyện</span></div><div className="month-header"><div><span className="kicker">TẬP {String(safeMonth).padStart(2,"0")} / 12</span><h1>{MONTHS[safeMonth - 1]}: Mỗi ngày một câu chuyện</h1><p>Chọn một ngày để nghe, đọc và khám phá nội dung truyện.</p></div><div className="month-progress"><strong>{list.filter(s => completedIds.has(s.id)).length}/{DAYS_IN_MONTH[safeMonth - 1]}</strong><span>Ngày đã hoàn thành</span><div className="progress-track"><div style={{ width: `${list.filter(s => completedIds.has(s.id)).length / DAYS_IN_MONTH[safeMonth - 1] * 100}%` }}/></div></div></div>
...
function ActivityPanel({ activities }: { activities?: StoryActivities }) {
  ...
  const hasContent = Boolean(activities.cloze_text || activities.true_false.length || activities.short_answer.length || activities.discussion.length);
  ...
  return <section className="activity-panel">
    ...
    {activities.true_false.length > 0 && <details><summary>Đúng hay sai? · {activities.true_false.length} câu</summary><ol className="activity-list">{activities.true_false.map((prompt, index) => <li key={index}><span>{prompt}</span><div className="tf-options"><button type="button" className={trueFalseAnswers[index] === true ? "selected" : ""} onClick={() => setTrueFalseAnswers(prev => ({ ...prev, [index]: true }))}>Đúng</button><button type="button" className={trueFalseAnswers[index] === false ? "selected" : ""} onClick={() => setTrueFalseAnswers(prev => ({ ...prev, [index]: false }))}>Sai</button></div></li>)}</ol></details>}
    {activities.short_answer.length > 0 && <details><summary>Trả lời ngắn · {activities.short_answer.length} câu</summary><ol className="activity-list">{activities.short_answer.map((prompt, index) => <li key={index}>{prompt}</li>)}</ol></details>}
...
function Admin({ stories, editor, setEditor, onSave, onDelete, onReload, email }: { stories: Story[]; editor: Story | null; setEditor: (story: Story | null) => void; onSave: (story: Story) => Promise<void>; onDelete: (story: Story) => Promise<void>; onReload: () => Promise<void>; email: string | null }) {
  const router = useRouter();
...
function StoryEditor({ story, stories, onClose, onSave, onDelete }: { story: Story; stories: Story[]; onClose: () => void; onSave: (story: Story) => Promise<void>; onDelete: (story: Story) => Promise<void> }) {
